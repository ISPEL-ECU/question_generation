...
getSetNotations <- function(leftIncl = FALSE, rightIncl = FALSE, leftBorder = sample(1:10, 1, replace = FALSE), rightBorder = sample(11:20, 1, replace = FALSE), membersType = 1, notation = 2, format = TRUE) {
  ...
  if (membersType == 2) {
    ...
    membersType <- "\\mathbb{R}"
  }
  resultSet <- getSeqSets(Sequence = 1, leftBorder4Generation, rightBorder4Generation, membersType)
  
  if (notation == 1) {
    resultSetString <- resultSet
    answerSet[[1]] <- resultSetString
    if (format == TRUE) {
      answerSet[[1]] <- formatListAsSet(answerSet[[1]])
    }
    else {
      answerSet[[1]] <- formatPartitionAsSet(answerSet[[1]])
    }
  }
  else if (notation == 2) {
    resultSetString <- paste('x \\in', membersType, '|', leftBorder, (if(leftIncl == TRUE) '\\le x' else '< x')  
                             , (if(rightIncl == TRUE) '\\le ' else '<'), rightBorder)
    answerSet[[1]] <- resultSetString
    if (format == TRUE) {
      answerSet[[1]] <- formatListAsSet(answerSet[[1]])
    }
